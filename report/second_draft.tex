...
% Reset page numbering to Arabic numerals starting from 1
...
In this simplified model, the \textbf{Value of Information} provided by the AI system is approximately \textbf{8.96 Million NOK per pen per cycle}.\footnote{This calculation assumes fixed prices, deterministic mortality rates, and immediate, perfect execution of management actions. Real-world outcomes are subject to price volatility, stochastic disease dynamics, and organizational response delays. See Section~7.3 for discussion of these uncertainties.} Even if the system is only 50\% effective, the ROI remains substantial, justifying the investment in ``Quality 4.0'' technologies \citep{Tsung2023, 10.1108/TQM-11-2023-0361}.
...
\begin{enumerate}
    \item \textbf{RQ1 (Model Performance):} What classification accuracy, recall, and precision can CNN architectures achieve for binary (Fresh vs.\ Infected) salmon disease detection on the SalmonScan dataset, and do deeper architectures provide significant improvements over shallow ones?
...
\begin{itemize}
    ...
    \item \textbf{Binary Classification Only:} The dataset groups all pathologies into a single \emph{Infected} class. It does not distinguish between specific diseases like PD, ISA, or winter ulcers. Consequently, the model can only serve as a general alert system, not a diagnostic tool.
    ...
    \item \textbf{Inverted Class Imbalance:} A critical deployment concern is that the training dataset exhibits a class imbalance \emph{opposite} to real-world conditions. In the SalmonScan dataset, infected fish constitute 64\% of samples, whereas in a healthy, well-managed sea pen, infected fish may represent fewer than 1\% of the population \citep{NVI_2025_NFHR2024}. This inversion creates a base rate fallacy problem: even with high Recall and Specificity, the Positive Predictive Value (PPV) in deployment will be substantially lower than the Precision observed in testing. For example, screening 100,000 fish with 0.5\% true infection rate using a model with 97.7\% Recall and 97\% Specificity would yield approximately 1,500 false alarms alongside 489 true positives, resulting in a PPV of only 24.6\%. Mitigation strategies include threshold adjustment to increase specificity, two-stage screening workflows, and continuous model retraining on operational data (see Section~\ref{sec:deployment}).
    \item \textbf{Environmental Bias (Domain Shift):} Perhaps the most critical technical limitation is the ``Land vs. Sea'' domain shift. The training images appear to be taken ``dry'' (out of water), likely at a processing plant or during a manual check. However, the target operational environment is underwater in sea pens. Underwater images suffer from light attenuation, scattering, and turbidity \citep{JIAN2021116088}. A model trained on clear, dry images will likely degrade significantly when applied to underwater footage without advanced domain adaptation techniques.
\end{itemize}
...
To ensure statistical rigor and quantify the variance due to random initialization and data splitting, we repeated the entire training process with \textbf{30 different random seeds} (0 through 29). Each seed controls both the train-test split and the neural network weight initialization, thereby capturing two sources of stochastic variation. The final performance metrics reported are the mean and standard deviation across all 30 runs, providing a robust measure of expected performance and its uncertainty on unseen data.

This approach offers several advantages over K-Fold Cross-Validation for our setting: (1) it provides a clean, held-out test set that is never used during training or hyperparameter tuning, (2) the multiple seeds capture variance from both data partitioning and model initialization, (3) 30 seeds provide sufficient samples to estimate variance reliably, and (4) it enables identification of \emph{consistently difficult images}---samples that are misclassified across multiple seeds and models, revealing systematic failure modes rather than random noise.

\subsection{Use of Artificial Intelligence in Research Process}
In accordance with academic transparency requirements, we disclose the use of AI assistance throughout this research project. Specifically, AI language models (primarily GitHub Copilot and ChatGPT) were employed in two capacities:

\begin{enumerate}
    \item \textbf{Code Development:} AI tools assisted in writing and debugging Python code for data preprocessing, model implementation, training loops, and evaluation metrics. While the overall architecture and methodology were designed by the authors, AI assistance accelerated implementation by suggesting code patterns, identifying syntax errors, and providing documentation for TensorFlow/Keras functions.
    \item \textbf{Text Formulation:} AI tools were used to help formulate and refine sections of this report, particularly for improving clarity, grammar, and academic tone. All technical content, analysis, interpretation of results, and conclusions were developed by the authors. AI suggestions were critically reviewed and edited to ensure accuracy and alignment with our research objectives.
\end{enumerate}

The final responsibility for all code functionality, methodological choices, results interpretation, and written content remains entirely with the authors. All AI-assisted content was verified against primary sources and validated through independent analysis.
...
\begin{tabular}{lccc}
\toprule
\textbf{Architecture} & \textbf{Conv Blocks} & \textbf{Filters per Block} & \textbf{Total Parameters} \\
\midrule
Model 1 (Shallow) & 1 & 32 & $\sim$1.2M \\
Model 2 (Medium) & 2 & 32, 64 & $\sim$2.8M \\
Model 3 (Deep) & 3 & 32, 64, 128 & $\sim$5.1M \\
\bottomrule
\multicolumn{4}{l}{\footnotesize All models use $3\times3$ convolutional kernels, ReLU activation, and $2\times2$ max pooling.}
\end{tabular}
\end{table}
...
The quantitative results for all models are summarized in Table \ref{tab:performance}. The performance varies across seeds, with the Ensemble achieving the best overall results across the 30 seed experiments.
...
\begin{table}[H]
\centering
\caption{Comparative Performance Metrics (Mean $\pm$ Std Across 30 Seeds)}
...
\begin{figure}[H]
    ...
    \caption{Confusion matrices showing mean values across 30 seeds for the four model architectures.}
...
\begin{figure}[H]
    ...
    \caption{Statistical comparison of model performance across 30 seeds, showing the distribution of metrics with error bars representing standard deviation.}
...
\begin{figure}[H]
    ...
    \caption{ROC curves comparing all three CNN architectures across 30 seeds with mean AUC values. Model 1 achieves the highest mean AUC (0.990), while Models 2 and 3 show comparable performance (0.979). The overlapping confidence bands indicate no significant differences between architectures.}
...
\textbf{Note on Process Capability Index ($C_{pk}$):} The Process Capability Index $C_{pk}$ is a standard Six Sigma metric used in manufacturing to measure how well a process fits within specification limits, assuming normally-distributed continuous measurements. However, $C_{pk}$ does not translate meaningfully to binary classification tasks where metrics are derived from aggregated confusion matrices across multiple train-test splits. In such contexts, the Sigma level itself serves as the primary capability indicator, directly reflecting the process yield (Recall) and providing an intuitive measure of quality that operations managers can monitor using control charts \citep{montgomery2020introduction}.
...
\begin{table}[H]
\centering
\caption{Six Sigma Quality Metrics (Mean Across 30 Seeds)}
\label{tab:sixsigma}
\begin{tabular}{lcccc}
\toprule
...
\begin{figure}[H]
    ...
    \caption{Six Sigma quality analysis: (a) Sigma level comparison, (b) False Negatives, and (c) Recall for each model.}
    \label{fig:six_sigma}
\end{figure}

\textbf{Interpretation:}
\begin{itemize}
    \item \textbf{All models achieve approximately 3$\sigma$ capability:} With 30 seeds providing a robust estimate, none of the models---including the Ensemble---reaches the 4$\sigma$ threshold. The Ensemble achieves the highest Sigma level (3.50$\sigma$) for infected detection, while individual models range from 3.01$\sigma$ (Model 3) to 3.53$\sigma$ (Model 2).
    \item \textbf{Model 2} achieved the best individual Infected Sigma level (3.53$\sigma$), slightly outperforming the Ensemble on this metric alone. However, Model 2 suffers from a higher False Positive rate (mean 1.1), resulting in the lowest Overall Sigma (3.13$\sigma$).
    ...
    \item \textbf{The Ensemble provides the best balance:} While Model 2 achieves marginally higher Infected Sigma (3.53$\sigma$ vs 3.50$\sigma$), the Ensemble's superior Overall Sigma (3.44$\sigma$ vs 3.13$\sigma$) and substantially lower variance make it the more robust choice for deployment. The Ensemble maintains a significantly lower False Positive rate (0.3 vs 1.1) while achieving comparable Recall, resulting in better balanced performance across both classes and greater stability across random seeds.
...
The first research question asked what classification performance CNN architectures can achieve on the SalmonScan dataset, and whether deeper architectures provide significant improvements over shallow ones.

The CNN architectures achieved strong performance, with mean Recall ranging from 93.5\% (Model~3) to 97.9\% (Model~2) and mean Accuracy from 93.2\% (Model~3) to 97.4\% (Ensemble) across 30 random seeds (Table~\ref{tab:performance}). Notably, no significant differences were observed among the three individual architectures. The overlapping confidence intervals (standard deviations of 5.8--12.0\% for Recall) indicate that architectural depth---varying from one to three convolutional blocks---does not materially affect mean performance on this dataset.
...
We propose integrating the model into a ``Screen-and-Refer'' workflow designed to augment, not replace, human decision-making. This workflow minimizes the risk of automation bias while maximizing throughput. Critically, this design also addresses the inverted class imbalance issue identified in Section~3.3: by requiring human confirmation of flagged cases before action, the system mitigates the impact of false alarms that would otherwise dominate in low-prevalence operational settings.
...
\begin{enumerate}
    ...
    The model assigns a ``Probability of Infection'' ($P_{inf}$) to each image. To address the base rate fallacy arising from low infection prevalence in healthy pens, the classification threshold $\tau$ may be adjusted upward from the standard 0.5 to increase specificity at the cost of modest recall reduction.
    \begin{itemize}
        \item If $P_{inf} < \tau$: Classify as \textbf{\emph{Fresh}}. No action.
        \item If $P_{inf} \ge \tau$: Flag as \textbf{Potential Infection}.
    \end{itemize}
    ...
    Confirmed cases trigger pen-level actions (e.g., lice treatment, antibiotic feed). Crucially, the expert's decision is fed back into the system to retrain and improve the model, creating a ``Human-in-the-Loop'' learning cycle. This continuous retraining on operational data helps adapt the model to the true class distribution and mitigates the training-deployment imbalance.
\end{enumerate}
...
This report has demonstrated that Convolutional Neural Networks can detect external disease symptoms in Atlantic salmon with reasonable precision and recall on a benchmark dataset. Through 30 random train-test splits, the Ensemble achieved a mean Recall of 97.7\% for infected fish detection, corresponding to approximately a 3.5$\sigma$ process capability level. The individual CNN architectures showed no significant differences, with the Ensemble's advantage coming from variance reduction through averaging.

\textbf{The predefined success criteria were met:} The Ensemble achieved Recall of 97.7\% ($\ge$95\% target), F1-Score of 97.9\% ($\ge$95\% target), and Infected Sigma level of 3.50$\sigma$ ($\ge$3.0$\sigma$ target), confirming that the model is fit-for-use as a decision support tool within the proposed human-in-the-loop workflow.
