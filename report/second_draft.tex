...
\begin{itemize}
    ...
    \item \textbf{Model 3 (Deep):} The deepest architecture, with three stacked Convolutional Blocks. This design enables the extraction of hierarchical features, crucial for distinguishing complex pathologies \citep{Hasan2022, HaddadMohammed2025}.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{ROC curves comparing all three CNN architectures across 30 seeds with mean AUC values. Model 1 achieves the highest mean AUC (0.990), while Models 2 and 3 show comparable performance (0.979). The overlapping confidence bands indicate no statistically significant differences between architectures.}
...
\begin{itemize}
    \item \textbf{All models achieve approximately 3$\sigma$ capability:} With 30 seeds providing a more robust estimate, none of the models---including the Ensemble---reaches the 4$\sigma$ threshold previously reported with fewer seeds. The Ensemble achieves the highest Sigma level (3.50$\sigma$) for infected detection, while individual models range from 3.01$\sigma$ (Model 3) to 3.53$\sigma$ (Model 2).
    ...
    \item \textbf{The Ensemble provides the best balance:} While not achieving the highest Infected Sigma, the Ensemble has zero mean False Negatives across 30 seeds and the best Overall Sigma (3.44$\sigma$), making it the most reliable choice for deployment.
...
Our study aimed to determine if CNNs are "fit-for-use" for salmon disease detection.
\begin{itemize}
    \item \emph{Is one model performing differently?} As shown in Section 6, the individual CNN architectures showed no statistically significant differences due to overlapping confidence intervals. The \textbf{Ensemble} achieved the best results through variance reduction, demonstrating the value of model averaging.
    \item \emph{Are Six Sigma tools helpful?} Yes, but with important caveats. Translating "97.7\% Recall" to "3.5$\sigma$" communicates that the process is adequate for deployment with human oversight but not for fully automated decision-making. The multi-seed approach provides confidence intervals on these metrics, enabling risk-informed deployment decisions.
\end{itemize}
...
Based on the multi-seed evaluation results, the \textbf{Ensemble Model} remains the recommended choice for deployment, though the margins are smaller than initially observed with fewer seeds. It achieved the best mean Recall (97.7\%), the best Overall Sigma level (3.44$\sigma$), and critically, zero mean False Negatives across 30 seeds. The soft voting mechanism provides an inherent confidence measure: when all three constituent models agree, confidence is high; disagreement signals borderline cases that may warrant human review.

From a Six Sigma perspective, with 30 seeds providing a more robust estimate, none of the models crosses the 4$\sigma$ threshold that traditionally indicates a highly reliable process. The Ensemble achieves approximately 3.5$\sigma$, which is adequate for deployment \emph{with human oversight} but necessitates the ``screen-and-refer'' workflow described below rather than fully automated decision-making.

An important finding from the 30-seed analysis is that \textbf{the three individual models show no statistically significant differences} in mean performance. The high variance across seeds (standard deviations of 5--12\%) means their confidence intervals overlap substantially. This suggests that for this small dataset, architectural complexity (1 vs. 2 vs. 3 convolutional blocks) does not materially affect average performance---the limiting factor is the data, not the model capacity.
